	...
	for t in range(10000) :
		...
		for i in range(M):
			for j in range(N):
				...
				b[i,j] -= 12
				for z in range(neighborsCoordinate):
					b[neighborsCoordinate[z][0],neighborsCoordinate[z][1]] += numToGive
		numExchanges+= 1

	if (numExchanges < 10000):
		f.write('Case #' + str(M) + ':  ' + str(numExchanges) + ' turns\n')
	else:
		f.write('Case #' + str(M) + ':  ' + str(numChildren) + ' children will play forever') 
